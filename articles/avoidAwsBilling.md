...
- WANリンクであるGigabitEthernetのインターフェースと既存の家庭用ルーターを接続する
...
![SplitNetwork1](images/SplitNetwork1.png)
 ...
 この構成ならスイッチを使うまでもなく、ルーターを二重に設置している時点でネットワークは分割されているようなものだから、単に追加したルーターにLANケーブルでデバイスを接続すれば事足りるのではないかと思われるかもしれませんが、せっかくスイッチを購入したので、導入してみました。

 スイッチを導入することで得られる利点は今回の場合、ポート数が増える以上には無いと思いますが、それに関しては、今後どうするか考えます。
...
C:\Users\PC_Usertracert google.com
